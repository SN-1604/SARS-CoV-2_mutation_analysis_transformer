...
链路: 周面板扰动 -> log1p/z -> repr13 周窗表征 -> 按窗结束月平均 -> 3种子残差集成预测
...
def main():
    ...
    def predict(Em_samples):
        """Em_samples: (N,K,64); 返回预测熵 (N,)"""
        idx = val
        xh = ((np.log1p(Xh[idx]) - y_mu) / y_sd * Xm[idx]).astype(np.float32)
        xm = Xm[idx][:, :, None].astype(np.float32)
    ...
    print(f"anchor R2 = {r2:.4f} (expect 0.8223)", flush=True)
    ...
    json.dump(dict(anchor_r2=float(r2), deltas=DELTAS, groups=GROUPS,
              ...
              open("factor_effects_monthly.json", "w"), ensure_ascii=False, indent=1)
    import csv
    with open("factor_effects_monthly.csv", "w", newline="", encoding="utf-8") as f:
    ...
    json.dump(dict(n_subset=int(sub.sum()), curves=va_res),
              open("factor_effects_vaccine_era.json", "w"), ensure_ascii=False, indent=1)
    print("疫苗时代子样本曲线已存 factor_effects_vaccine_era.json")
